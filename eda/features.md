...
| order_day_of_week         | orders                  | `order_purchase_timestamp.dt.dayofweek`                     | 주문 요일 (0=월)                         |
...
| category_avg_delay_days   | orders + products           | `groupby product_category_name: 평균 지연 일수` | 카테고리별 평균 지연 일수         |
...
- 데이터 리키지 주의 피처: `state_avg_delay_days`, `seller_avg_delay_days`는 예측 시점 이전 기준만 사용
- 변수 타입 태그: 범주형(Categorical), 연속형(Numerical), 이진형(Boolean) 표시하면 모델 입력 정리에 도움
- 파생 관계 표시: 예) `late_by_days`는 `actual_delivery_days`와 `estimated_delivery_days`의 파생

## 결론

- 실무적으로 구현 가능한 피처만 선별되어 있으며, 해석력과 구현 용이성 모두 우수함
- 위 구조를 기반으로 피처 엔지니어링 → 모델링 → 중요도 분석까지 자연스럽게 이어질 수 있음
