    ...
    try:
        ...
        for avsc_filename in src_avsc_filenames:
            ...
            deserialized_stream = BaseStream.get_topic_df(
                get_kafka_stream_df(spark_session, dst_avsc_reader.table_name),
                dst_avsc_reader
            )
...
def load_medallion_layer(micro_batch_df:DataFrame, batch_id: int):
    ...
    for topic_name in BronzeAvroSchema.get_all_filenames():
        try:            
            ...
            if record_count == 0:
                print(f"[{dst_avsc_reader.dst_table_identifier}] No records")
                continue
            ...
            print(f"[{dst_avsc_reader.dst_table_identifier}] has saved {record_count} rows (Processing time: {end-start:.2f} sec)")

        except Exception as e:
            print(f"[{dst_avsc_reader.dst_table_identifier}] {e}")
