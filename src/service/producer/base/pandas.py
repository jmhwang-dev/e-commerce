...
class PandasProducer(BaseProducer):
    ...
    @staticmethod
    def get_mock_ingest_time(log: pd.DataFrame, current_time:pd.Timestamp) -> Tuple[Optional[pd.DataFrame], Optional[pd.Timestamp]]:
        """
    ...
    @staticmethod
    def add_mock_ingest_time(log: pd.DataFrame, timestamp_col: str) -> Optional[pd.DataFrame]:
        """
        개발 편의성을 위해 컨슈머가 아닌 프로듀서에서 이벤트 생성 시간을 의미하는 임의의 `ingest_time`을 모방한 시간을 추가함.
        - 이유: `micro_batch`에서 이벤트 시간이 없는 메시지의 `ingest_time`을 컨슈머에서 모방하기 복잡하여 개발 편의성을 높이고자 프로듀서가 임의로 `ingest_time`을 추가함.
        - 실제 운영시에는 컨슈머에서 토픽을 소비하는 시점에 실제 시간을 `ingest_time`으로 사용함.
        ...
        HOW-TO
        - `event timestamp` 레코드를 기준으로 주입 시간 생성
