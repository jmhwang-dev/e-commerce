...
* **Goal:** 실시간으로 배송 지연을 감지하고, 상품 등급(Product Portfolio Matrix)에 따른 우선순위별 조치를 지원하여 운영 효율을 극대화합니다.
* **Approach:** **Lambda Architecture**를 도입하여 운영 이슈 대응을 위한 실시간성(Speed Layer)과 정밀한 분석을 위한 신뢰성(Batch Layer)을 동시에 확보했습니다.
