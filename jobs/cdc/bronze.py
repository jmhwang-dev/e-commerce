...
if __name__ == "__main__":
    ...
    cdc_topic_names = BronzeTopic.get_cdc_topics()
    cdc_topic_stream_df = get_kafka_stream_df(spark_session, cdc_topic_names)
    base_stream_df = cdc_topic_stream_df.select(col("key").cast("string"), col("value"), col("topic"))

    avsc_reader = AvscReader(cdc_topic_names[0])
