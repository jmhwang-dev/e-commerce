...
if __name__=="__main__":
    ...
    for i, order_status_series in order_status_df.iterrows():
        ...
        mock_order_status_series, current_ingest_time = PandasProducer.get_mock_ingest_time(order_status_series, current_event_timestamp)
        ...
        if status == 'purchase':
            ...
            mock_payment_log, current_ingest_time = PandasProducer.get_mock_ingest_time(payment_log, current_ingest_time)
            PaymentBronzeProducer.publish(mock_payment_log)

            order_item_log = OrderItemBronzeProducer.select(order_status_series, ['order_id'])
            mock_order_item_log, current_ingest_time = PandasProducer.get_mock_ingest_time(order_item_log, current_ingest_time)
            OrderItemBronzeProducer.publish(mock_order_item_log)

            proudct_log = ProductBronzeProducer.select(order_item_log, ['product_id'])
            mock_proudct_log, current_ingest_time = PandasProducer.get_mock_ingest_time(proudct_log, current_ingest_time)
            ProductBronzeProducer.publish(mock_proudct_log)

            seller_log = SellerBronzeProducer.select(order_item_log, ['seller_id'])
            mock_seller_log, current_ingest_time = PandasProducer.get_mock_ingest_time(seller_log, current_ingest_time)
            SellerBronzeProducer.publish(mock_seller_log)

            geolcation_seller = GeolocationBronzeProducer.select(seller_log, ['zip_code'])
            mock_geolcation_seller, current_ingest_time = PandasProducer.get_mock_ingest_time(geolcation_seller, current_ingest_time)
            GeolocationBronzeProducer.publish(mock_geolcation_seller)

            customer_log = CustomerBronzeProducer.select(order_status_series, ['customer_id'])
            mock_customer_log, current_ingest_time = PandasProducer.get_mock_ingest_time(customer_log, current_ingest_time)
            ...
            mock_geolcation_customer, current_ingest_time = PandasProducer.get_mock_ingest_time(geolcation_customer, current_ingest_time)
            GeolocationBronzeProducer.publish(mock_geolcation_customer)

        review_log = ReviewBronzeProducer.select(current_event_timestamp)
        mock_review_log = PandasProducer.add_mock_ingest_time(review_log, 'review_creation_date')
